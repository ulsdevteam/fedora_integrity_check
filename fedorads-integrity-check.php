...
$objectStore_rows = get_objectStore_rows($conn);

/**
 * Loop through the objectStore records to perform tuque calls on the underlying
 * objects.
 */
for ($offset = 1; $offset <= $objectStore_rows; $offset++) {
  ...
  try {
    ...
    foreach ($object as $obj_ds) {
      ...
      if ($datastream) {
        ...
        $ret['dsChecksumType'] = isset($ds_info['dsChecksumType']) ? $ds_info['dsChecksumType'] : '';
...
function save_datastream_record($osa, $conn) {
  ...
  $sql = "INSERT INTO datastreamStore (`PID`, `dsid`, `dsVersionID`, " .
         "`dsLabel`, `dsCreateDate`, `dsMIME`, " .
         "`dsSize`, `dsLocation`, `dsChecksum`, `dsChecksumType`, " .
         ...
         "('" . addslashes($osa['PID']) . "', '" . addslashes($osa['dsid']) . "', '" . addslashes($osa['dsVersionID']) . "', '" .
          ...
          $osa['dsSize'] . ", '" . addslashes($osa['dsLocation']) . "', '" . addslashes($osa['dsChecksum']) . "', '" . addslashes($osa['dsChecksumType']) . "', " .
...
function save_object_store_record($osa, $conn) {
  ...
  $sql = "INSERT INTO objectStore (`offset`, `PID`, `Label`, `models`, `Owner`, `timestamp`, `problem`) VALUES " .
